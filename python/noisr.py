...
class Api:
    def __init__(self):
        self.base_url = 'http://api.noisr.pt'
    ...
    def get_numbers(self, count, bits=8) -> list:
        """Get a list of random numbers from the API."""
        try:
            response = self.__post('generate', {"count": count, "bits": bits}, headers={'Content-Type': 'application/json'})
